...
const CommonUtils = {
    ...
    /**
     * Shared mobile pickup helper.
     * @param {PointerEvent} e - The pointer event.
     * @param {Object} gameState - The current game state.
     * @param {Object} dragState - The current drag state.
     * @param {Object} options - Callbacks and configurations.
     */
    handleMobilePickup: function (e, gameState, dragState, options) {
        const cardEl = e.target.closest('.card');
        const targetPileEl = e.target.closest('.pile, .tableau-column, .foundation-pile, .freecell-slot, .freecell-foundation, .spider-column');

        // 1. If we already have a pickup, try to drop it
        if (dragState.pickedUpCard) {
            // If tapping the same card again, or tapping an invalid area, cancel pickup
            if (cardEl === dragState.pickedUpElement) {
                this.clearPickup(dragState);
                return true;
            }

            // Attempt to drop at the tapped location
            const clientX = e.clientX;
            const clientY = e.clientY;
            const success = options.onAttemptDrop(clientX, clientY);

            if (success) {
                this.clearPickup(dragState);
            } else {
                // If it wasn't a valid drop, and we didn't tap a new movable card, cancel pickup
                if (!cardEl || !options.isMovable(cardEl)) {
                    this.clearPickup(dragState);
                } else {
                    // Tap on another potentially movable card - swap pickup?
                    // For now, let's keep it simple and just clear pickup.
                    this.clearPickup(dragState);
                    // Then pick up the new one if movable
                    this.pickUpCard(cardEl, dragState, options);
                }
            }
            return true;
        }

        // 2. If nothing is picked up, try to pick up a card
        if (cardEl && options.isMovable(cardEl)) {
            this.pickUpCard(cardEl, dragState, options);
            return true;
        }
    ...
    /**
     * Logic to "pick up" a card/sequence.
     */
    pickUpCard: function (cardEl, dragState, options) {
        const sequence = options.getSequence(cardEl);
        if (!sequence || sequence.length === 0) return;

        dragState.draggedCards = sequence;
        dragState.pickedUpCard = sequence[0];
        dragState.pickedUpElement = cardEl;
        dragState.pickedUpSource = options.getSource(cardEl);
        dragState.draggedElements = options.getElements(cardEl);

        dragState.draggedElements.forEach(el => el.classList.add('picked-up'));
        this.playSound('card');
    },

    /**
     * Clears the current pickup state.
     */
    clearPickup: function (dragState) {
        if (dragState.draggedElements) {
            dragState.draggedElements.forEach(el => el.classList.remove('picked-up'));
        }
        ...
        // Also clear any game-specific source info if stored directly in dragState
        if (dragState.sourcePile !== undefined) dragState.sourcePile = null;
        if (dragState.sourceIndex !== undefined) dragState.sourceIndex = null;
        if (dragState.source !== undefined) dragState.source = null;
    }
};
